...
- GitHub Pages / 純前端第二版會在切割補面上產生母榫孔；它是專門針對切面定位孔的演算法，不是通用 CAD boolean。
...
注意：純前端第二版會在切割面挖圓柱母孔，適合目前的定位 pin 用途。它不是完整 mesh boolean，所以孔位仍建議避開模型外框、薄壁和功能面，輸出後請在切片軟體檢查。
